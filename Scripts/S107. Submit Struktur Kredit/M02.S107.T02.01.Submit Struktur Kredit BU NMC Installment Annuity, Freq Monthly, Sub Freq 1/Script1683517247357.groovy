...
TestData excelData = findTestData('Data Files/Submit Struktur Kredit BU NMC')
...
for(int i = 1; i <= excelData.getRowNumbers(); i++) {
	...
	String nomorBpkb = excelData.getValue('Bpkb Number', i)
	String tanggalBpkb = excelData.getValue('Bpkb Date', i)
	...
	CustomKeywords.'id.co.fifgroup.fifcore.submitStrukturKredit.inputDetailOrder'(installment, frequecnyMonth, frequencyNumber, calculatedBy, interestRateEffective, interestRateFlat, netAdmin, promoFee, advanceArrear, paymentMethod)
	...
	CustomKeywords.'id.co.fifgroup.fifcore.submitStrukturKredit.objectFinanceBpkbAddress'(provinsi, kota, kecamatan, kelurahan, alamat, rt, rw, nomorBpkb, tanggalBpkb)
...
//	WebUI.sendKeys(null, Keys.ENTER, FailureHandling.OPTIONAL)
//	
